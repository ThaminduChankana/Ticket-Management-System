...
public class ReentrantLockTicketPoolBenchmarkTest {
    private static final int CAPACITY = 1000;
    private static final int OPS_PER_THREAD = 10_000;
    private static final int[] THREAD_COUNTS = {2, 4, 16, 64};

    @Test
    public void benchmarkReentrantLockTicketPool() throws InterruptedException {
        System.out.println("=== ReentrantLockTicketPool Benchmark ===");
        for (int tc : THREAD_COUNTS) {
            double time = measure(new ReentrantLockTicketPool(CAPACITY), tc, OPS_PER_THREAD);
