...
public class BlockingQueueTicketPoolBenchmarkTest {
    private static final int CAPACITY = 1000;
    private static final int OPS_PER_THREAD = 10_000;
    private static final int[] THREAD_COUNTS = {2, 4, 16, 64};

    @Test
    public void benchmarkBlockingQueueTicketPool() throws InterruptedException {
        System.out.println("=== BlockingQueueTicketPool Benchmark ===");
        for (int tc : THREAD_COUNTS) {
            double time = measure(new BlockingQueueTicketPool(CAPACITY), tc, OPS_PER_THREAD);
